...
class HotelApp:
    ...
    def login_gui(self):
        ...
        # ✅ REĢISTRĀCIJA AR VALIDĀCIJU
        def register():
            pw = password.get()

            if len(pw) < 8:
                messagebox.showerror("Kļūda", "Parolei jābūt vismaz 8 simboliem!")
                return

            if not re.search(r"[A-Z]", pw):
                messagebox.showerror("Kļūda", "Parolei jābūt vismaz vienam LIELAJAM burtam!")
                return

            if not re.search(r"[!@#$%^&*(),.?\":{}|<>]", pw):
                messagebox.showerror("Kļūda", "Parolei jābūt vismaz vienam speciālam simbolam!")
                return
    ...
    def make_reservation(self):
        ...
        def save():
            ...
            if self.cursor.fetchone():
                messagebox.showerror("Kļūda", "Numurs jau ir rezervēts šajā laikā!")
                return
